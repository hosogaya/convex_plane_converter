...
int main()
{
    ...
    if (!convex_plane::ConvexPlaneConverter::toMessge(A, b, C, d, normal, message))
    {
        std::cout << "toMessage failed" << std::endl;
        return 1;
    }
    printMultiArray(message.a_matrix, "A matrix");
    printMultiArray(message.b_vector, "b vector");
    printMultiArray(message.c_matrix, "C matrix");
    printMultiArray(message.d_vector, "d vector");
    printMultiArray(message.normal, "normal");
